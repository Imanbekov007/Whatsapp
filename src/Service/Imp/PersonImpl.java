...
public class PersonImpl implements PersonService {
    private List<Passport> passportList = new ArrayList<>();    public List<Passport> getPassportList(){
        return passportList;    }

    public void setPassportList(List<Passport> passportList){
        this.passportList = passportList;    }
    @Override    public List<Passport> getAllPassports() {
        return passportList;    }

    @Override    public List<Passport> getPassportByFirstName() {
        List<Passport> result = new ArrayList<>();
        System.out.print("First Name : ");
        String getfirstName  = new Scanner(System.in).nextLine();
        for (Passport p : passportList)
            if(p.getName().equals(getfirstName))
                result.add(p);
        return result;    }

    @Override    public String createPassport() {
        boolean a = false;        try {
            System.out.print("ID : ");
            ...
            String gender = new Scanner(System.in).next().toUpperCase();
            System.out.print("Country : ");
            String country = new Scanner(System.in).next().toUpperCase();
            LocalDate dateOfBirth = LocalDate.of(
                    Integer.parseInt(birthDateList[0]),
                    Integer.parseInt(birthDateList[1]),
                    Integer.parseInt(birthDateList[2]));
            Passport passport = new Passport(
                    id, firstName,lastname,firstName, Gender.valueOf(gender), Country.valueOf(country),dateOfBirth);
